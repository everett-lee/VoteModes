...
class MonthWithIntervals:
    ...
    def get_first_interval(self, month, year) -> DateInterval:
        ...
        first_open = '{year}-{month}-01'.format(year=year, month=month_str) if month == 1 \
    ...
    def get_third_interval(self, month, year) -> DateInterval:
        ...
        third_close = '{year}-{month}-01'.format(year=year, month=next_month_str) if \
            month < 12 else '{year}-12-31'.format(year=year)
